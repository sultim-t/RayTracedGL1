...
typedef struct RgStaticTextureCreateInfo
{
    ...
    // If data is null, then the library will try to find
    // image file that was specified by "filePath".
    uint32_t                *data;
    // Path to the file must be in the following format:
    // "<path to a folder>/<file name>.<file extension>".
...
typedef enum RgDrawFrameFlagBits
{
    ...
    RG_DRAW_FRAME_ALL_ROUGHNESS_ONE         = 1 << 3,
    RG_DRAW_FRAME_ALL_ROUGHNESS_ZERO        = 1 << 4,
    RG_DRAW_FRAME_ALL_METALLICITY_ONE       = 1 << 5,
    RG_DRAW_FRAME_ALL_METALLICITY_ZERO      = 1 << 6,
} RgDrawFrameFlagBits;
